...
import byteware.ui.callback.GuiInput;
...
public class SliderComponent implements Component {
	...
	public void draw(AtomicInteger offset) {
		...
		String formattedValue = EnumChatFormatting.AQUA + this.slider.getValueString();
		...
		Gui.drawRect(sliderStart, this.parentModule.category.getY() + this.offsetY + 11, sliderEnd, this.parentModule.category.getY() + this.offsetY + 15, new Color(100, 0, 200).getRGB());

		GL11.glPushMatrix();
		GL11.glScaled(0.5D, 0.5D, 0.5D);

		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(this.slider.getName() + ": " + formattedValue, (float) ((int) ((float) (this.parentModule.category.getX() + 4) * 2.0F)), (float) ((int) ((float) (this.parentModule.category.getY() + this.offsetY + 3) * 2.0F)), -1);
	...
	public boolean isLeftHalfHovered(int x, int y) {
		return x > this.x &&
				x < this.x + this.parentModule.category.getWidth() / 2 + 1 &&
				y > this.y + 8 &&
				y < this.y + 16;
	}
