...
public class CheckBoxComponent implements Component {
	...
	public CheckBoxComponent(BooleanSetting setting, ModuleComponent parentModule, int offsetY) {
	...
	public void draw(AtomicInteger offset) {
		...
		String formattedValue = setting.value ?
				EnumChatFormatting.GREEN + "true" :
				EnumChatFormatting.RED + "false";

		Minecraft.getMinecraft().fontRendererObj.drawString(this.setting.name.replace("-", " ") + ": " + formattedValue, (float) ((this.module.category.getX() + 4) * 2), (float) ((this.module.category.getY() + this.offsetY + 5) * 2), -1, false);
	...
	public boolean isHovered(int x, int y) {
		return x > this.x &&
				x < this.x + this.module.category.getWidth() &&
				y > this.y &&
				y < this.y + 11;
	}
